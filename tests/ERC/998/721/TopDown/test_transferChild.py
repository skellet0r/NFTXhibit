...
def test_transfers_ownership_of_a_token(alice, bob, nft, xhibit):
    ...
    xhibit.transferChild(0, bob, nft, 0, {"from": alice})
...
def test_emits_TransferChild_event(alice, bob, nft, xhibit):
    tx = xhibit.transferChild(0, bob, nft, 0, {"from": alice})
...
def test_transferFrom_subcalls_child_contract(alice, bob, nft, xhibit):
    tx = xhibit.transferChild(0, bob, nft, 0, {"from": alice})

    assert tx.subcalls[0]["to"] == nft
    assert tx.subcalls[0]["function"] == "transferFrom(address,address,uint256)"
    assert tx.subcalls[0]["inputs"] == dict(_from=xhibit, _to=bob, _tokenId=0)
...
def test_reverts_if_sender_is_not_root_owner_operator_or_approved(bob, nft, xhibit):
    with brownie.reverts("dev: Caller is neither owner nor operator nor approved"):
        xhibit.transferChild(0, bob, nft, 0, {"from": bob})

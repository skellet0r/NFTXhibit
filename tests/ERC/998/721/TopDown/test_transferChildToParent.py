...
def local_setup(alice, composable_bottom_up, xhibit):
    composable_bottom_up.safeTransferFrom(alice, xhibit, 0, 0, {"from": alice})


def test_emits_TransferChild_event(alice, composable_bottom_up, nft, xhibit):
    tx = xhibit.transferChildToParent(
        0, nft, 0, composable_bottom_up, 0, b"", {"from": alice}
    )

    assert "TransferChild" in tx.events
    assert tx.events["TransferChild"]["_fromTokenId"] == 0
    assert tx.events["TransferChild"]["_to"] == nft
    assert tx.events["TransferChild"]["_childContract"] == composable_bottom_up
    assert tx.events["TransferChild"]["_childTokenId"] == 0


def test_subcalls_child_contract(alice, composable_bottom_up, nft, xhibit):
    tx = xhibit.transferChildToParent(
        0, nft, 0, composable_bottom_up, 0, b"", {"from": alice}
    )
    ...
    assert tx.subcalls[0]["to"] == composable_bottom_up
    assert (
